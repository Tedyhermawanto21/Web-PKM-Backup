...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-slate-500 text-sm">Informasi lengkap kelompok yang meminta Anda sebagai pembimbing.</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div class="p-3 bg-slate-50 rounded-xl border border-slate-100">
                        <p class="text-xs text-slate-500 uppercase font-semibold">Tanggal Permintaan</p>
        ...
        <div class="space-y-6">
            ...
            <!-- Status Permintaan (Actions) -->
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <h2 class="font-bold text-slate-800 mb-4">Status Permintaan</h2>
                
                @if ($kelompok->status == 'submitted')
                    <div class="space-y-3">
                    <div class="space-y-3">
                        <form id="form-accept-{{ $kelompok->id }}" action="{{ route('dosen.kelompok_requests.accept', $kelompok->id) }}" method="POST">
                            @csrf
                        ...
                        <form id="form-reject-{{ $kelompok->id }}" action="{{ route('dosen.kelompok_requests.reject', $kelompok->id) }}" method="POST">
                            @csrf
    ...
    <script>
        function confirmAccept(id) {
            Swal.fire({
                title: 'Terima Kelompok?',
                text: "Anda akan menjadi Dosen Pembimbing untuk kelompok ini.",
                icon: 'question',
        ...
        function confirmReject(id) {
            Swal.fire({
                ...
                text: "Permintaan bimbingan ini akan ditolak.",
                icon: 'warning',
                showCancelButton: true,
                ...
                @else
                    <div class="text-center mb-4">
                     ...
                     <div class="p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
                        <p class="text-sm text-slate-500">Tindakan sudah dilakukan pada {{ $kelompok->updated_at->format('d/m/Y') }}</p>
                    </div>
                @endif
            </div>
        </div>
