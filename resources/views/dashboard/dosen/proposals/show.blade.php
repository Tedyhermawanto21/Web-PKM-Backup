...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-slate-500 text-sm">Tinjau detail dan ambil keputusan verifikasi.</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                ...
                @php
                    $canAction = ($proposal->status == 'menunggu_approval' || $proposal->status_dosen == 'menunggu' || $proposal->status_dosen == 'pending');
                    $isApproved = ($proposal->status == 'disetujui' || $proposal->status_dosen == 'disetujui');
                    $isRejected = ($proposal->status == 'ditolak' || $proposal->status_dosen == 'ditolak');
                @endphp

                @if ($canAction && !$isApproved && !$isRejected)
                    <div class="space-y-3">
                        <form action="{{ route('dosen.pengajuan_kelompok_pkm.approve', $proposal->id) }}" method="POST">
                            ...
                            <button type="submit"
                                ...
                                onclick="return confirm('Apakah Anda yakin ingin menyetujui proposal ini dan menjadi dosen pembimbing?')">
             ...
             @if ($proposal->catatan_penolakan)
                <div class="bg-red-50 rounded-2xl shadow-sm border border-red-100 p-6">
                    <h5 class="font-bold text-red-800 mb-2">Catatan Penolakan</h5>
                    <p class="text-red-700 text-sm">{{ $proposal->catatan_penolakan }}</p>
                </div>
            @endif
        </div>
    </div>
