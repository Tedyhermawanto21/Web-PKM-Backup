...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                <div class="p-4 bg-slate-50 rounded-lg">
                    ...
                    <p class="mt-4 text-sm text-slate-500">Status Dosen</p>
                    <p class="font-bold">{{ ucfirst($kelompok->status) }}</p>

                    <p class="mt-4 text-sm text-slate-500">Status Kaprodi</p>
                    <p class="font-bold">{{ ucfirst($kelompok->status_kaprodi ?? 'menunggu') }}</p>
                    ...
                    <div class="mt-6">
                        ...
                        @if ($dosenApproved && $kaprodiPending)
                            <form action="{{ route('kaprodi.kelompok_requests.accept', $kelompok->id) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="w-full inline-flex items-center justify-center px-4 py-2 rounded-lg bg-uhamka-500 text-white font-bold">Setujui</button>
                            </form>
                        ...
                        @else
                            <div class="text-sm text-slate-600">Tindakan sudah dilakukan atau menunggu persetujuan dosen.
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="mt-6">
            <a href="{{ route('kaprodi.kelompok_requests.index') }}" class="text-uhamka-500 font-bold">← Kembali ke
                daftar</a>
        </div>
    </div>
@endsection
