...
@section('content')
    ...
    @if ($user->proposalsAsDosen()->where('status', 'menunggu_approval')->count() > 0)
        <div
            class="mb-8 p-4 rounded-2xl bg-uhamka-yellow-50 border border-uhamka-yellow-200 flex items-start sm:items-center gap-4 shadow-sm animate-pulse-slow">
            <div
                class="w-10 h-10 rounded-full bg-uhamka-yellow-100 flex items-center justify-center text-uhamka-yellow-600 flex-shrink-0">
            ...
            <div class="flex-1">
                ...
                <p class="text-sm text-uhamka-yellow-700">
                    Anda memiliki <span
                        class="font-bold">{{ $user->proposalsAsDosen()->where('status', 'menunggu_approval')->count() }}
                        proposal</span> yang menunggu review Anda.
                </p>
            </div>
            <a href="{{ route('dosen.pengajuan_kelompok_pkm.index') }}"
                class="px-4 py-2 bg-uhamka-yellow-400 text-uhamka-900 font-bold text-sm rounded-lg hover:bg-uhamka-yellow-500 transition-colors shadow-sm">
                Review Sekarang
            </a>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <!-- Menunggu Approval -->
        <div
            class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-uhamka-yellow-200 transition-all group">
            <div class="flex items-start justify-between mb-4">
                ...
                <span
                    class="px-3 py-1 bg-orange-50 text-orange-600 text-xs font-bold rounded-lg uppercase tracking-wider">Menunggu
                    Approval</span>
            </div>
            <div class="flex items-baseline gap-2">
                <span
                    class="text-4xl font-extrabold text-slate-900">{{ $user->proposalsAsDosen()->where('status', 'menunggu_approval')->count() }}</span>
            </div>
        </div>
        ...
        <div
            ...
            <div class="flex items-baseline gap-2">
                <span
                    class="text-4xl font-extrabold text-slate-900">{{ $user->proposalsAsDosen()->where('status', 'disetujui')->count() }}</span>
            </div>
        </div>
        ...
        <div
            ...
            <div class="flex items-baseline gap-2">
                <span
                    class="text-4xl font-extrabold text-slate-900">{{ $user->proposalsAsDosen()->where('status', 'ditolak')->count() }}</span>
            </div>
        </div>
    </div>
