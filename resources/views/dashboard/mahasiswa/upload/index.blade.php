...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
        ...
        @if ($uploadSchedule)
            <a href="{{ route('mahasiswa.upload.create') }}" class="inline-flex items-center px-4 py-2 bg-uhamka-900 text-white text-sm font-bold rounded-xl hover:bg-uhamka-800 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
        ...
        @else
            <button class="inline-flex items-center px-4 py-2 bg-slate-200 text-slate-400 text-sm font-bold rounded-xl cursor-not-allowed" disabled>
    ...
    @if ($uploadSchedule)
        <div class="mb-6 p-4 rounded-xl bg-green-50 border border-green-200 flex items-start gap-4 animate-fade-in-down shadow-sm">
            ...
            <div>
                ...
                <div class="flex items-center gap-2 text-sm font-mono text-green-800 bg-green-100 px-3 py-1 rounded-lg inline-flex">
                ...
                @if ($uploadSchedule->description)
                    <p class="text-xs text-green-600 mt-2 italic border-t border-green-200 pt-2">{{ $uploadSchedule->description }}</p>
                @endif
            </div>
    ...
    @else
        <div class="mb-6 p-4 rounded-xl bg-yellow-50 border border-yellow-200 flex items-start gap-4 animate-fade-in-down shadow-sm">
            ...
            <div>
                ...
                <p class="text-sm text-yellow-700">Mohon tunggu pengumuman dari admin untuk jadwal upload proposal berikutnya.</p>
            </div>
        </div>
    @endif
    ...
    @if ($hasRevision && $revisionSchedule)
        <div class="mb-6 p-4 rounded-xl bg-blue-50 border border-blue-200 flex items-start gap-4 animate-fade-in-down shadow-sm">
            ...
            <div>
                <h4 class="font-bold text-blue-800 mb-1">Jadwal Revisi Tahap {{ $hasRevision->revision_stage }} Dibuka!</h4>
                 <div class="flex items-center gap-2 text-sm font-mono text-blue-800 bg-blue-100 px-3 py-1 rounded-lg inline-flex mt-1">
                ...
                @if ($revisionSchedule->description)
                     <p class="text-xs text-blue-600 mt-2 italic border-t border-blue-200 pt-2">{{ $revisionSchedule->description }}</p>
                @endif
            </div>
    ...
    @elseif($hasRevision)
        <div class="mb-6 p-4 rounded-xl bg-orange-50 border border-orange-200 flex items-start gap-4 animate-fade-in-down shadow-sm">
            ...
            <div>
                 <h4 class="font-bold text-orange-800 mb-1">Jadwal Revisi Tahap {{ $hasRevision->revision_stage }} Belum Dibuka</h4>
                <p class="text-sm text-orange-700">Proposal Anda memerlukan revisi. Mohon tunggu jadwal revisi dibuka oleh admin.</p>
            </div>
        </div>
    @endif
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if ($proposal->status_admin == 'menunggu')
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-yellow-100 text-yellow-700 animate-pulse">Menunggu Review</span>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('mahasiswa.upload.show', $proposal->id) }}"
                                            class="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors shadow-sm" title="Lihat Detail">
                                        ...
                                        @if ($proposal->status_admin == 'ditolak' || $proposal->revision_stage > 0)
                                            ...
                                            @if ($canEdit)
                                                <a href="{{ route('mahasiswa.upload.edit', $proposal->id) }}"
                                                    class="p-2 bg-yellow-50 text-yellow-600 rounded-lg hover:bg-yellow-100 transition-colors shadow-sm" title="Edit / Revisi">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                </a>
                                            @else
                                                <button class="p-2 bg-slate-100 text-slate-400 rounded-lg cursor-not-allowed" disabled
        ...
        @else
            <div class="flex flex-col items-center justify-center py-12 text-slate-400">
                <svg class="w-16 h-16 mb-4 text-slate-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                ...
                <a href="{{ route('mahasiswa.upload.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-uhamka-500 text-white text-sm font-bold rounded-xl hover:bg-uhamka-600 transition-all">
                    Upload Proposal PKM
                </a>
            </div>
        @endif
