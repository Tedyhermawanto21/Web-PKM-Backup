...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Edit Proposal PKM</h1>
            <p class="text-slate-500">Perbarui informasi proposal PKM Anda.</p>
        </div>
    ...
    @if ($proposal->status == 'ditolak' && $proposal->catatan_penolakan)
         <div class="mb-6 p-4 rounded-xl bg-red-50 border border-red-200 animate-fade-in-down">
            <strong class="text-red-800 block mb-1 flex items-center">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-6 border-b border-slate-100 bg-slate-50">
            <h6 class="font-bold text-slate-800">Formulir Edit Proposal PKM</h6>
        </div>
        <div class="p-8">
            <form action="{{ route('mahasiswa.proposals.update', $proposal->id) }}" method="POST">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                     <select class="w-full rounded-xl border-slate-300 bg-slate-50 text-slate-800 focus:bg-white focus:border-uhamka-500 focus:ring-4 focus:ring-uhamka-100 transition-all duration-200 py-3 px-4 shadow-sm appearance-none @error('skema') border-red-500 focus:ring-red-100 @enderror" id="skema" name="skema" required>
                                        ...
                                        <option value="PKM-KC" {{ old('skema', $proposal->skema) == 'PKM-KC' ? 'selected' : '' }}>PKM-KC (Karsa Cipta)</option>
                                        <option value="PKM-RE" {{ old('skema', $proposal->skema) == 'PKM-RE' ? 'selected' : '' }}>PKM-RE (Riset Eksakta)</option>
                                        <option value="PKM-GT" {{ old('skema', $proposal->skema) == 'PKM-GT' ? 'selected' : '' }}>PKM-GT (Gagasan Tertulis)</option>
                                        <option value="PKM-AI" {{ old('skema', $proposal->skema) == 'PKM-AI' ? 'selected' : '' }}>PKM-AI (Artikel Ilmiah)</option>
                                        <option value="PKM-PM" {{ old('skema', $proposal->skema) == 'PKM-PM' ? 'selected' : '' }}>PKM-PM (Pengabdian Masyarakat)</option>
                                        <option value="PKM-K" {{ old('skema', $proposal->skema) == 'PKM-K' ? 'selected' : '' }}>PKM-K (Kewirausahaan)</option>
                                        <option value="PKM-VGK" {{ old('skema', $proposal->skema) == 'PKM-VGK' ? 'selected' : '' }}>PKM-VGK (Video Gagasan Konstruktif)</option>
                                    </select>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <input type="text"
                                            ...
                                            name="anggota[{{ $i }}][nim]"
                                            value="{{ old('anggota.' . $i . '.nim', $anggota->nim ?? '') }}" placeholder="NIM anggota"
                                            required>
                                        @error('anggota.' . $i . '.nim')
                                             <p class="mt-2 text-xs text-red-600">{{ $message }}</p>
                                        @enderror
                ...
                <div class="flex items-center gap-4 pt-8 border-t border-slate-100">
                    ...
                    <a href="{{ route('mahasiswa.proposals.show', $proposal->id) }}" class="inline-flex items-center px-8 py-4 bg-white text-slate-600 font-bold text-lg rounded-xl border border-slate-200 hover:bg-slate-50 hover:text-slate-800 transition-all">
