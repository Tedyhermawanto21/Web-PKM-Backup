    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-uhamka-200 transition-all group">
            ...
            <div class="flex items-baseline gap-2">
                <span class="text-4xl font-extrabold text-slate-900">{{ $proposals->where('status_admin', 'menunggu')->count() }}</span>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-green-200 transition-all group">
            ...
            <div class="flex items-baseline gap-2">
                <span class="text-4xl font-extrabold text-slate-900">{{ $proposals->where('status_admin', 'disetujui')->count() }}</span>
                <span class="text-sm font-bold text-slate-400">Proposal</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-red-200 transition-all group">
            ...
            <div class="flex items-baseline gap-2">
                <span class="text-4xl font-extrabold text-slate-900">{{ $proposals->where('status_admin', 'ditolak')->count() }}</span>
                <span class="text-sm font-bold text-slate-400">Proposal</span>
            </div>
        </div>
    </div>
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{ Storage::url($proposal->file_proposal) }}" target="_blank"
                                        class="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors inline-block text-center" title="Download">
                                        <svg class="w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                    </a>
                                </td>
                                <td class="px-6 py-4 text-center">
