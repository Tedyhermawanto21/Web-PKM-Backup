...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Edit Akun Kaprodi</h1>
            <p class="text-slate-500">Perbarui data Kepala Program Studi.</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div class="p-6">
                    <form action="{{ route('admin.kaprodis.update', $user) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" class="block w-full rounded-xl border-slate-300 focus:border-uhamka-500 focus:ring-uhamka-500 shadow-sm py-3 px-4 @error('name') border-red-500 text-red-900 placeholder-red-300 focus:ring-red-500 focus:border-red-500 @enderror" id="name" name="name" value="{{ old('name', $user->name) }}" placeholder="Nama lengkap beserta gelar" required>
                                @error('name')
                                     <p class="mt-2 text-sm text-red-600 flex items-center gap-1"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <!-- Password -->
                                <div>
                                    <label for="password" class="block text-sm font-bold text-slate-700 mb-2">Password Baru <span class="text-slate-400 font-normal">(Opsional)</span></label>
                                    <input type="password" class="block w-full rounded-xl border-slate-300 focus:border-uhamka-500 focus:ring-uhamka-500 shadow-sm py-3 px-4 @error('password') border-red-500 text-red-900 placeholder-red-300 focus:ring-red-500 focus:border-red-500 @enderror" id="password" name="password" placeholder="Isi untuk mengubah password">
                                    @error('password')
                                         <p class="mt-2 text-sm text-red-600 flex items-center gap-1"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="password" class="block w-full rounded-xl border-slate-300 focus:border-uhamka-500 focus:ring-uhamka-500 shadow-sm py-3 px-4" id="password_confirmation" name="password_confirmation" placeholder="Ulangi password baru">
                                </div>
                            </div>
                        </div>
            ...
            <div class="bg-blue-50 rounded-2xl p-6 border border-blue-100">
                ...
                <ul class="space-y-3 text-sm text-blue-800">
                    <li class="flex items-start gap-2">
                        ...
                        <span><strong>Perubahan Data:</strong> Data yang diubah akan langsung berlaku setelah disimpan.</span>
                    </li>
                </ul>
            </div>
        </div>
